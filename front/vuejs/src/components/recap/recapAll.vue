...
<div class="recap">
    <div class="ui grid">
        <div v-if='(this.storeuser[0])' class="row">
            <div class="one wide column"></div>

            <div class="four wide column"> <!-- Historique produits gaspillés -->
                <router-link :to="{name : 'Processing'}"><h1 class="header ui centered">In my fridge</h1></router-link>
                <div class="ui celled grid">
                    <div class="row ui selection list" v-for="aliment in currentaliments" :key="aliment.id"  style="background-color:#e6f9ff" >
            ...
            <div class="four wide column">
                <router-link :to="{name : 'Waste'}"><h1 class="header ui centered">My wastes</h1></router-link>
                <div class="ui celled grid">
                    <div class="row ui selection list" v-for="aliment in wastealiments" :key="aliment.id" style="background-color:#e6f9ff">
            ...
            <div class="four wide column">
                <router-link :to="{name : 'Eaten'}"><h1 class="header ui centered">My eaten products</h1></router-link>
                <div class="ui celled grid">
                    <div class="row ui selection list" v-for="aliment in eatenaliments" :key="aliment.id" style="background-color:#e6f9ff">
                        <div v-if="aliment.status==3 && aliment.user_id==storeuser[0].id" class="item"> 
                                <div class="ui image">
                                    <img v-if="aliment.image" class="ui avatar image" :v-bind:src="aliment.image" alt="food img">
                                    <i v-else class="leaf icon"></i>
                                </div>
        ...
        <div class="row">
            ...
            <div class="five wide column"><highcharts class="hc" :options="chartOptions" ref="chart"></highcharts></div>
            <div class="six wide column"><highcharts class="hc" :options="chartOptions2" ref="chart"></highcharts></div>
            <div class="five wide column"><highcharts class="hc" :options="chartOptionsLine" ref="chart"></highcharts></div>
...
export default {
    ...
    data() {
      return {
            ...
            xmonth:[],
            chartOptions: {
                chart: {type: 'pie', backgroundColor:"#e6f9ff"},
                title: {text: 'Your stats'},
                series: [{name: 'Current stat', data: []}]
            },
            chartOptions2: {
                chart: {type: 'pie', backgroundColor:"#e6f9ff"},
                title: {text: 'Your wastes by category'},
                series: [{name: 'Proportion of wastes', data: []}]
            },
             chartOptionsLine: {
                chart: {type: 'spline', backgroundColor:"#e6f9ff"},
                xAxis: {categories: []},
                yAxis: [{title: {text: 'Money waste ($$)'}},{title: {text: 'Number of waste'},opposite: true,}],
                title: {text: 'Your money wastes these last six months!!'},
    ...
    methods : {
        ...
        getPieCount(){
            ...
            if(this.inprocess != 0)
                this.chartOptions.series[0].data.push({y:this.inprocess, name:"In my fridge", color:"#ffd966"})
        ...
        getThirty(){
            this.array = this.$store.state.kitchen.reverse()
            this.array.forEach(aliment => {
                if(this.wastealiments.length < 10 && aliment.status == 2 && aliment.user_id == this.storeuser[0].id)
                   this.wastealiments.push(aliment)
                else if(this.currentaliments.length < 10 && aliment.status == 0 && aliment.user_id == this.storeuser[0].id)
                    this.currentaliments.push(aliment)
                else if(this.eatenaliments.length < 10 && aliment.status == 3 && aliment.user_id == this.storeuser[0].id)
                    this.eatenaliments.push(aliment)
            })
        },
